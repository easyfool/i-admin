...
var addTabs = function (options) {
    ...
    if (findTabPanel(pageId) === null) {
        ...
        if (options.content) {
            ...
            var $content = null;
            if(options.targetType==="ajax-tab"){
                console.log("url:"+options.url);
                $content = $('<div></div>').css("width", "100%").attr("id", "iframe_" + pageId).attr(pageIdField, pageId);;

                $content.load(options.url,function(response,status,xhr){
            ...
            else if(options.targetType==="iframe-tab"){
                options.url = basePath + options.url;
    ...
    $.fn.sidebarMenu = function (options) {
        ...
        function init($menu_ul, data, level) {
            $.each(data, function (i, item) {
                ...
                //如果不是header
                var li = $('<li role="presentation"  class="treeview " data-level="' + level + '"></li>');

                //a标签
                var $a;
                if (level > 0) {
                    $a = $('<a role="tab" data-toggle="tab" style="padding-left:' + (level * 20) + 'px"></a>');
                } else {
                    $a = $('<a></a>');
                }
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    if (item.targetType != null && item.targetType === "blank") //代表打开新页面
                    {
                        ...
                        var href = 'addTabs({targetType:\''+item.targetType+'\',id:\'' + item.id + '\',title: \'' + item.text + '\',close: true,url: \'' + item.url + '\',urlType: \'' + item.urlType + '\'});';
                        $a.attr('onclick', href);
                    }
                    else if(item.targetType != null && item.targetType === "ajax-tab"){
                        item.urlType = item.urlType ? item.urlType : 'relative';
                        var href = 'addTabs({ targetType:\''+item.targetType+'\',id:\'' + item.id + '\',title: \'' + item.text + '\',close: true,url: \'' + item.url + '\',urlType: \'' + item.urlType + '\'});';
                        $a.attr('onclick', href);
                    }
